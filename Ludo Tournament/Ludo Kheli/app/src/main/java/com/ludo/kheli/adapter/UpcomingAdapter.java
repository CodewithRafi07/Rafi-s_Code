...
public class UpcomingAdapter extends RecyclerView.Adapter<UpcomingAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if (!dataArrayList.get(position).getParti1_name().equals("0") && !dataArrayList.get(position).getParti2_name().equals("0")) {
            ...
            holder.titleTv.setText(String.format("%s Vs Player Need 2", dataArrayList.get(position).getParti1_name()));
        }
            ...
            holder.titleTv.setText(String.format("%s Vs Need Player 1", dataArrayList.get(position).getParti1_name()));
        }
        ...
        if (dataArrayList.get(position).getType() != 0) {
            ...
            if (dataArrayList.get(position).getTable_joined() >= dataArrayList.get(position).getTable_size()) {
                ...
                holder.roomStatusTv.setText("মাত্র "+leftSize+" জন বাকি");
                holder.roomSizeTv.setText(String.format("1vs2 Player: %d/%d", dataArrayList.get(position).getTable_joined(), dataArrayList.get(position).getTable_size()));
            }
        }
        else {
    ...
    private void calculateTime(long milliSeconds, TextView timerTv, Button joinBt, TextView roomSizeTv, TextView roomStatusTv, ProgressBar progressBar, int position) {
        if (milliSeconds != 0) {
            ...
            if (dataArrayList.get(position).getType() != 0) {
                roomStatusTv.setText(String.format("মাত্র %d জন বাকি", dataArrayList.get(position).getTable_size()));
